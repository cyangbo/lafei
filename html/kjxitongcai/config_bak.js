...
exports.cp=[
	/* 
	{
		...
		option:{
			host:"localhost",
			timeout:50000,
			path: '/index.php?s=/home/wufencai/info0',
			headers:{
		...
		parse:function(str){
			try{
				return getFromWANJINYULE(str,36);//36 为秒秒彩的ID编号
			}catch(err){
				throw('分分彩解析数据不正确');
			}
		}
				...
				for(var go=0;go<1000000;go++){
					if(yingLv>wfGaiLv){//如果开奖概率大于{wfGaiLv=后台设置的开奖概率},则随机开号,小于的话{平台必须赢}
						...
						bets.forEach(function(bet){//forEach枚举计算每一笔投注返奖金额
							...
							win+=bet.bonusProp * zjCount * bet.beiShu * bet.mode/2;//返奖总额
							all+=bet.mode*bet.beiShu*bet.actionNum;                //投注总额
						});
