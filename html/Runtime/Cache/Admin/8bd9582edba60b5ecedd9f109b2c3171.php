...
<body>
    ...
    <tbody>
		<?php if($_list): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				<?php
 $path = explode(',' , $vo['parents']); foreach($path as $p){ echo $members_list[$p].'>'; } ?>
			</td>
			...
			<td>
				<?php if(!$vo['state']){ ?>
				<a target="modal" width="360" title="到帐处理" modal="true" button="确定:dataAddCode|取消:defaultCloseModal" href="<?php echo U('Business/toOn_recharge?id='.$vo['id'].'&username='.$vo['username'].'&amount='.$vo['amount']);?>">到帐处理</a>
				<?php } ?>
