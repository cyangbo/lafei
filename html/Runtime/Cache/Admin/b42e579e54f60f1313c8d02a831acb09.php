...
<body>
	...
	<form action="<?php echo U('index');?>" method="post">
		<div class="cf">			
			<a class="btn" href="<?php echo U('User/add_2');?>">新 增</a>
 ...
 if( $vo['loginIP']>0 && $vo['accessTime']>0 && (time()-$vo['accessTime'])<C('SESSION_TIME') ) echo '<font color="#FF0000">在线</font>'; else echo '离线'; ?>
			...
			<td><a href="<?php echo U('User/index?parentId='.$vo['uid']);?>">下级</a>
				...
				<?php if(($vo["isDelete"]) == "0"): ?><a href="<?php echo U('User/changeStatus?method=deleteuser&id='.$vo['uid']);?>" class="confirm ajax-get">删除</a>
				<?php else: ?>
				<a href="<?php echo U('User/changeStatus?method=undeleteuser&id='.$vo['uid']);?>" class="confirm ajax-get">恢复</a><?php endif; ?>
                </td>
