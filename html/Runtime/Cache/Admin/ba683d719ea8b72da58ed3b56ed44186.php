...
<body>
	...
	<form action="/lafei/html/admin.php?s=/admin/time/edit.html" method="post" class="form-horizontal">
		<div class="form-item">
			<label class="item-label">时间设置<span class="check-tips">（开奖时间，只能为时间格式）</span></label>
			<div class="controls">
				<input type="text" class="text input-large" name="name" value="<?php echo ((isset($info["actionTime"]) && ($info["actionTime"] !== ""))?($info["actionTime"]):''); ?>">
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		//导航高亮
		$('.side-sub-menu').find('a[href="<?php echo U('Time/index');?>"]').closest('li').addClass('current');
	</script>
