...
<body>
	...
	<?php $mname=array( '2.00'=>'元', '0.20'=>'角', '0.02'=>'分' ); $liqTypeName=array( 1=>'充值', 2=>'返点', 5=>'停止追号', 6=>'中奖金额', 7=>'撤单', 8=>'提现失败返回冻结金额', 9=>'管理员充值', 10=>'解除抢庄冻结金额', 50=>'签到赠送', 51=>'首次绑定工行卡赠送', 52=>'充值佣金', 53=>'消费佣金', 100=>'抢庄冻结金额', 101=>'投注冻结金额', 102=>'追号投注', 103=>'抢庄返点金额', 105=>'抢庄赔付金额', 106=>'提现冻结', 107=>'提现成功扣除冻结金额', 108=>'开奖扣除冻结金额' ); ?>
	<!-- 高级搜索 -->
	<form action="<?php echo U('adminlog');?>" method="post">
		<div class="search-form fr cf">
			<div class="sleft">
				管理员：<input style="width:100px" type="text" name="username" class="search-input" value="<?php echo I('username');?>" placeholder="用户名">
			</div>
			<div class="sleft">
				<select style="width:120px;" name="type">
					<option value="">所有操作类型</option>
					<?php if(is_array($adminLogType)): $key = 0; $__LIST__ = $adminLogType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($key % 2 );++$key;?><option value="<?php echo ($key-1); ?>" title="<?php echo ($value); ?>"><?php echo ($value); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	...
	<script type="text/javascript">
		...
		$('.side-sub-menu').find('a[href="<?php echo U('Action/adminlog');?>"]').closest('li').addClass('current');
	</script>
