...
<body>
	...
	<ul class="tab-nav nav">
		<?php foreach($chiTypes as $key=>$var){ ?>
			<li <?=$this->iff($type==$key, 'class="current"','')?>><a href="<?php echo U('?type='.$key);?>"><?=$var?></a></li>
		<?php } ?>
	</ul>


	<div class="data-table table-striped">
		<?php foreach($groups as $group) { $url = U('config/switchPlayedGroupStatus'); ?>
		<table>
			<thead>
				<tr>
					...
					<th><?php if($group['enable']) echo "<a target='ajax' method='post' href='".U('config/switchPlayedGroupStatus')."'><font color='#ffffff'>关闭</font></a>"; else echo "<a target='ajax' method='post' href='".U('config/switchPlayedGroupStatus')."'><font color='#ffffff'>开启</font></a>"; ?></th>
				</tr>
			</thead>
			<tbody>
				<?php foreach($playeds as $played){ if($played['groupId']==$group['id']){ ?>
					<tr>
						...
						<td>开/关：<input type="checkbox" name="enable" <?php echo ($this->iff($played['enable'],'checked','')); ?> ></td>
						<td width="10%"><a href="<?php echo U('config/switchPlayedStatus');?>" target="ajax" method="post">保存</a> </td>
					</tr>
				<?php }} ?>
			</tbody>
		</table>
	...
	<script type="text/javascript">
        //导航高亮
        $('.side-sub-menu').find('a[href="<?php echo U('Config/playList');?>"]').closest('li').addClass('current');
    </script>
