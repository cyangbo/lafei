...
<body>
	...
	<div class="cf">
		<a class="btn" href="<?php echo U('config/addBank');?>">新 增</a>
		<!-- 高级搜索 -->
		...
		<table>
			...
			<tbody>
				<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><tr>
						<td><?php echo ($config["bankName"]); ?></td>
						<td><?php echo ($config["account"]); ?></td>
						<td><?php echo ($config["username"]); ?></td>
						<td><?php echo ($this->iff($config['enable'],'开','关')); ?></td>
						<td>
							<a class="confirm ajax-get" title="删除" href="<?php echo U('config/delBank?id='.$config['id']);?>">删除</a>
						</td>
