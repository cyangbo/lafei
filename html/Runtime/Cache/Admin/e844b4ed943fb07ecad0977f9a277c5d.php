...
<body>
	...
	<form action="<?php echo U('index');?>" method="post">
		<div class="cf">			
			<a class="btn" href="<?php echo U('User/add_admin');?>">新 增</a>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><span><?php echo date('Y/m/d H:i:s', $vo['loginTime']);?></span></td>
			<td><?php
 if( $vo['loginIP']>0 && $vo['accessTime']>0 && (time()-$vo['accessTime'])<C('SESSION_TIME') ) echo '<font color="#FF0000">在线</font>'; else echo '离线'; ?>
			</td>
			<td>
				<?php if(($vo["username"]) == "admin"): ?><a href="<?php echo U('User/add_admin?id='.$vo['uid']);?>">修改密码</a>
				<?php else: ?>
				<a href="<?php echo U('User/add_admin?id='.$vo['uid']);?>">修改密码</a>
				<?php if(($vo["isDelete"]) == "0"): ?><a href="<?php echo U('User/del_admin?isDelete=1&id='.$vo['uid']);?>" class="confirm ajax-get">删除</a>
				<?php else: ?>
				<a href="<?php echo U('User/del_admin?isDelete=0&id='.$vo['uid']);?>" class="confirm ajax-get">恢复</a><?php endif; endif; ?>
                </td>
    ...
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('User/admin_list');?>"]').closest('li').addClass('current');
	</script>
