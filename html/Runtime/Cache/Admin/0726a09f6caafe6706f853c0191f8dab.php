...
<body>
	...
	<?php $stateName=array('已到帐','申请中','已取消','已支付','已失败','已删除'); ?>
	<!-- 标题栏 -->
	...
	<form action="<?php echo U('cash');?>" method="post">
		<div class="search-form fr cf">
			...
			<div class="sleft">
				时间从：<input type="text" name="fromTime" id="first" class="search-input" value="<?=$this->iff(I('formTime'),I('formTime'),date('Y-m-d',time())) ?>" placeholder="请输入开始时间">
			</div>
			<div class="sleft">
    ...
    <tbody>
		<?php if($_list): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				<?php if($vo['state']==1){ ?>
				<a target="modal" width="420" title="提款处理" modal="true" button="确定:dataAddCode|取消:defaultCloseModal" href="<?php echo U('Business/to_cash?id='.$vo['id'].'&account='.$vo['account'].'&username='.$vo['username'].'&bankName='.$vo['name'].'&amount='.$vo['amount'].'&home='.$vo['home']);?>">处理</a>
				<?php } ?>
