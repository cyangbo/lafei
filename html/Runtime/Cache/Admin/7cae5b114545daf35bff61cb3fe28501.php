...
<body>
    ...
    <ul class="tab-nav nav">
        ...
        <li <?=$this->iff($type==$data['id'], 'class="current"','')?>>
            <a href="<?php echo U('?type='.$data['id']);?>">
                <?=$data['shortName']?>
            </a>
        </li>
        <?php } ?>
        ...
        <table>
            ...
            <tbody>
                <?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <input type="text" name="actionTime" value="<?php echo ($config["actionTime"]); ?>">
                            <input type="hidden" name="id" value="<?php echo ($config["id"]); ?>">
                        </td>
                        <td>
                            <a title="保存" href="<?php echo U('update');?>" target="ajax" method="post">保存</a>
                        </td>
    ...
    $(function() {
        ...
        //导航高亮
        $('.side-sub-menu').find('a[href="<?php echo U('
            Time / index ');?>"]').closest('li').addClass('current');
    });
    </script>
