...
<title>添加银行－<?php echo S('WEB_NAME');?></title>
	...
	<div id="page-content">
		<div class="panel">
			<div class="panel-heading">
				<h3 class="panel-title"><img alt="img" src="/lafei/html/Public/Home/images/fei_game/card.png">我的银行卡</h3>
			</div>
					...
					<div class="mminfo">
						...
						<div class="mminfobox">
							<div class="row">
								...
								<div class="col-md-4">
									                            当前还可以添加
									<font class="text-danger" style="font-size: 18px; font-weight: bold;"><?php echo (3-count($mybank));?> </font>张银行卡。
								</div>
							</div>
						</div>
					</div>					
					<div class="row banklist">
						<div class="col-md-4">
							<div class="bankbox addbank">
								<a id="addBank" href="javascript:;" <?php if(count($mybank)>2) echo 'no="1"'; ?>><i class="fa fa-plus"></i><span id="addBankInfo">添加新的银行卡</span></a>
							</div>
						</div>
						<?php foreach($mybank as $b){ ?>
							<div class="col-md-4">
								<div class="bankbox">
									<div class="bank_1020000">
										<img alt="" src="/lafei/html/Public/Home/images/fei_game/tx.jpg"><strong><?php echo ($banks[$b['bankId']]['name']); ?></strong>
										<label>尾号<?=preg_replace('/^.*(\w{4})$/', '\1', $b['account'])?></label>
										<em>持卡人：<?=mb_substr($b['username'],0,1,'utf-8').'**' ?></em>
									</div>
									<p>
										绑定时间：<?php echo date('Y/m/d H:i:s',$b['actionTime']);?>
									</p>
								</div>
							</div>
						<?php } ?>
							...
							<div class="row">
								<div class="col-sm-5">									
									<div id="j-validateFristRemark">
										<br>
										<br>
										<a class="text-2x text-primary text-bold" href="<?php echo U('user/password');?>">添加银行卡不需要验证。如资金密码没有设置，请点击我，直接进入资金密码设置。</a>
									</div>
								</div>
								<div class="col-sm-7">									
									<div id="addBankDiv" style="margin-left: 15px; display: block;">
										...
										<div style="text-align: center;">
											...
											<button class="btn btn-primary btn-labeled fa fa-lg" id="submitBtn" style="width: 150px; height: 35px;" type="button" action="<?php echo U('user/setCBAccount');?>">确    定</button>
...
<script src="/lafei/html/Public/Home/js/bankmanage.min.js"></script>
