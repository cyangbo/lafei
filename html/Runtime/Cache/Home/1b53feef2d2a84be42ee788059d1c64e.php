        ...
        <table width="100%" class="table table-striped" id="my-datatable" cellspacing="0">
            ...
            <tbody class="table_b_tr">
                ...
                <tr>
                    ...
                    <td>
                    <?php
 if($var['state'] == 1){ echo count($cashcount)-1 . '人在等待处理！'; }elseif($var['state'] ==2){ echo '已取消！'; }elseif($var['state'] ==3){ echo '已支付！'; }elseif($var['state'] ==4){ echo '提现失败！'; }elseif($var['state'] ==5){ echo '后台删除！'; }elseif($var['state'] ==0){ echo '确认到帐！'; } ?>
...
$(document).ready(function () {
	$("#j-query").on("click", function () {				
		$.ajax({
			type: "GET",
			url: '<?php echo U("team/searchCashRecord");?>',
