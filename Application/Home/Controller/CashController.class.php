...
class CashController extends HomeController
{
    ...
    public function cash()
    {
        ...
        if ($user['coinPassword'] != md5(I('FundsPass'))) {
            $this->error('资金密码不正确');
        }
...
//近2天来消费总额
        ...
        //dump(I('UserBankId'));exit;
        $bank = M('member_bank')->where(array('uid' => $this->user['uid'], 'bankId' => I('UserBankId')))->select();
...
// 流动资金
            ...
            if ($return) {
                M()->commit(); //成功则提交
                $this->success('申请提现成功，提现将在10分钟内到帐，如未到账请联系在线客服。', U('team/cashRecord'));
